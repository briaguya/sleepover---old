...
#				s-p-d			26 modifica prenotazione a seconda checkin: sempre - fino al checkin - dopo checkin
#				s-p-d			27 modifica prenotazione a seconda checkout: sempre - fino al checkout - dopo checkout
...
if ($modifica_doc == "s" or $modifica_doc == "n") $nuovi_priv_mod_pers = substr($nuovi_priv_mod_pers,0,2).$modifica_doc;
...
if (${"ins_commento".$anno_modifica} == "n") echo mex("L'utente",$pag)." <i>$nome_utente_privilegi</i> ".mex("non potrà inserire dei <b>commenti</b> quando inserisce nuove prenotazioni nel",$pag)." $anno_modifica.<br>";
if (${"ins_commento".$anno_modifica} == "s") echo mex("L'utente",$pag)." <i>$nome_utente_privilegi</i> ".mex("potrà inserire dei <b>commenti</b> quando inserisce nuove prenotazioni nel",$pag)." $anno_modifica.<br>";
...
if ($ins_origine_prenota == "n" or $ins_origine_prenota == "s") $nuovi_priv_ins_prenota = substr($nuovi_priv_ins_prenota,0,11).$ins_origine_prenota;
...
echo "<br><div style=\"text-align: center;\">".mex("Privilegi per l'anno",$pag)." <b>$anno_mostra</b></div>";
...
echo "".mex("Possibilità di inserire un commento",$pag).":
...
echo "".mex("Possibilità di modificare il commento",$pag).":
 <label><input type=\"radio\" name=\"modp_commento$anno_mostra\" value=\"s\"$checked_SI>$b_SI".mex("Si",$pag)."$b_slash_SI</label>
 <label><input type=\"radio\" name=\"modp_commento$anno_mostra\" value=\"n\"$checked_NO>$b_NO".mex("No",$pag)."$b_slash_NO</label><br>";
unset($checked_SI);
unset($b_SI);
unset($b_slash_SI);
unset($checked_NO);
...
if (substr($priv_mod_prenota,25,1) == "p") { $checked_PR = " checked"; $b_PR = "<b>"; $b_slash_PR = "</b>"; }
if (substr($priv_mod_prenota,25,1) == "d") { $checked_DO = " checked"; $b_DO = "<b>"; $b_slash_DO = "</b>"; }
...
if (substr($priv_mod_prenota,26,1) == "p") { $checked_PR = " checked"; $b_PR = "<b>"; $b_slash_PR = "</b>"; }
if (substr($priv_mod_prenota,26,1) == "d") { $checked_DO = " checked"; $b_DO = "<b>"; $b_slash_DO = "</b>"; }
