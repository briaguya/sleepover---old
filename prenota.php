...
${"tipo_commento".$n_t} = ${"tipo_commento".$num_tipologie};
...
$num_tipologie = risul_query($dati_transazione,0,'dati_transazione1');
$prenota_vicine = risul_query($dati_transazione,0,'dati_transazione12');
...
$tipo_commento = ${"tipo_commento".$n_t};
...
$costo_tariffap = (double) $costo_tariffap * $numpersone;
...
$prezzo_costo_tot = calcola_prezzo_totale_costo($dati_ca,$num_costo,$inizioperiodo,$fineperiodo,$settimane_costo,$moltiplica,$costo_tariffa,$lista_tariffe_sett,$costo_tariffa_tot,$caparra,$numpersone);
...
$settimane_costo = ${"settimane_costo".$numca."_".$n_t."t".$num1};
$moltiplica = ${"moltiplica".$numca."_".$n_t."t".$num1};
$prezzo_costo_vett[$num_costo] = calcola_prezzo_totale_costo($dati_ca,$num_costo,$inizioperiodo,$fineperiodo,$settimane_costo,$moltiplica,$costo_tariffa,$lista_tariffe_sett,$costo_tariffa_tot,$caparra,$numpersone,$costo_escludi_perc);
...
if ($tipo_commento == "checkin") echo " <em>".mex("per l'entrata",$pag)."</em>";
if ($tipo_commento == "checkout") echo " <em>".mex("per l'uscita",$pag)."</em>";
echo ": $commento<br>";
$commento = aggslashdb($commento);
if ($tipo_commento == "checkin") $commento = ">$commento>";
if ($tipo_commento == "checkout") $commento = ">>$commento";
...
<option value=\"tar\"$sel_tar>".mex("prezzo tarffa",$pag)."</option>
...
$sel_checkin = "";
$sel_checkout = "";
if (${"tipo_commento".$n_t} == "checkin") $sel_checkin = " selected";
if (${"tipo_commento".$n_t} == "checkout") $sel_checkout = " selected";
if (!$sel_checkin and !$sel_checkout) $sel_null = " selected";
echo " <select name=\"tipo_commento$n_t\">
<option value=\"\"$sel_null>".mex("per la prenotazione",$pag)."</option>
<option value=\"checkin\"$sel_checkin>".mex("per l'entrata",$pag)."</option>
<option value=\"checkout\"$sel_checkout>".mex("per l'uscita",$pag)."</option>
</select>";
} # fine if ($attiva_checkin == "SI")
echo ":<br>
 <textarea name=\"commento$n_t\" rows=3 cols=45 style=\"white-space: pre; overflow: auto;\">".${"commento".$n_t}."</textarea><br>";
