...
<h5>".mex("Pagina per controllare la disponibilità",$pag)."</h5><br><br>
...
if (!$aggiungi_costi_fissi or strtoupper($aggiungi_costi_fissi) == $SI) { $sel_SI = " selected"; $sel_NO = ""; }
else { $sel_NO = " selected"; $sel_SI = ""; }
...
if (C_RESTRIZIONI_DEMO_ADMIN == "SI") { $ind_email = C_EMAIL_DEMO_ADMIN; $readonly = " readonly=\"readonly\""; }
else $readonly = "";
...
".mex("Campi della form personalizzati",$pag).":<br>
...
if (!${"chiedi_num_doc_cond".$num1} or ${"chiedi_num_doc_cond".$num1} == $SI) { $sel_SI = " selected"; $sel_opz = ""; }
else { $sel_SI = ""; $sel_opz = " selected"; }
...
$js_opz_tema .= "".mex("Colore del tema",$pag)." \"".$template_theme_name[$num1]."\" ".mex("per",$pag)." \"".$colori_tema[$num2]['name']."\":\\
...
$js_opz_tema .= "".mex("Valore del tema",$pag)." \"".$template_theme_name[$num1]."\" ".mex("per",$pag)." \"".$valori_tema[$num2]['name']."\":\\
...
<input class=\"sbutton\" type=\"submit\" value=\"".mex("Crea la pagina per la disponibilità",$pag)."\">
</div></div></form><br>
...
<input class=\"sbutton\" type=\"submit\" name=\"form_modello_disponibilita\" value=\"".mex("Pagina per la disponibilità",$pag)."\">
</div>";
...
<input class=\"sbutton\" type=\"submit\" name=\"form_$template_name\" value=\"".$nome_modello_ext."\">
</div>";
} # fine if ($template_name_show['tpl_type'] != "interconnection")
} # fine if ($modello_ext != "." and $modello_ext != ".." and...
} # fine while ($file = readdir($lang_dig))
